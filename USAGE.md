...
- **Python**: PEP 508 environment markers are intentionally ignored (treated as always-true). License obligations don't depend on `python_version` or `sys_platform`, so missing a marker-gated copyleft would be a compliance gap. Extras are followed: with a `uv.lock`, packages installed through an extra (`django[argon2]` → `argon2-cffi`) come from the lockfile's `[package.optional-dependencies]` edges for every extra requested in the lock.
